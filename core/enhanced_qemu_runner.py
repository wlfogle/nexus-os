...
class AIEnhancedQEMURunner:
    ...
    def _load_iso_profiles(self) -> Dict[str, ISOProfile]:
        ...
        profiles = {
            ...
            'nexusos': ISOProfile(
                ...
                network_mode="virtio",
                boot_priority="d",
                special_flags=["-machine", "type=q35", "-cpu", "host"],
                description="NexusOS AI-Native Operating System (Pop!_OS based, NVIDIA GPU)"
            ),
            'popos': ISOProfile(
                name="Pop!_OS",
                category="Linux Distribution",
                ...
                boot_priority="d",
                special_flags=["-machine", "type=q35"],
                description="System76 Pop!_OS Linux distribution"
            )
        }
    ...
    def _identify_iso_file(self, iso_path: str) -> Tuple[str, ISOProfile]:
        ...
        patterns = {
            ...
            r'nexusos|nexus-os|nexus.os': 'nexusos',
            r'pop.os|popos|pop_os': 'popos'
        }
        ...
        if self._is_nvme_partition(boot_path):
            ...
            if '-bios' not in ' '.join(profile.special_flags or []):
                ...
                ovmf_paths = [
                    ...
                    '/usr/share/OVMF/OVMF_CODE.fd',         # Pop!_OS/Ubuntu alt path
                    '/usr/share/OVMF/OVMF_CODE_4M.fd'       # Pop!_OS 4M variant
                ]
