...
import { ComputerDesktopIcon, CircleStackIcon, GlobeAltIcon, ChartBarIcon } from '@heroicons/react/24/outline';
...
export default function Dashboard() {
 ...
 return runningVms.reduce((sum, vm) => sum + (vm.memory * 1024 * 1024), 0); // Assuming 1MB per VM
 };
 ...
 icon={ChartBarIcon}
 color="green"
 subtitle={`${Math.round(metrics.total_memory_used / 1024)}GB / ${Math.round(
 metrics.total_memory_available / 1024,
 )}GB`}
 />
 <MetricCard
 title="Storage Pools"
 ...
 <dd className="mt-1 text-sm text-gray-900 dark:text-white">
 {Math.round(hostInfo.memory_free / 1024)}GB free / {Math.round(
 hostInfo.memory_total / 1024,
 )}GB total
 </dd>
 </div>
 </div>
