...
# 🎮 Complete Diablo IV Gaming Setup
# This script implements everything needed to get Diablo IV working in your VM
...
echo -e "${BLUE}🎮 Complete Diablo IV Gaming Setup${NC}"
echo -e "${BLUE}===================================${NC}"
echo "VM Image: /var/lib/libvirt/images/win10-gaming-legacy.qcow2"
echo ""
...
# Check VM status
print_status "INFO" "Checking gaming VM status..."
if sudo virsh list --state-running | grep -q "win10-gaming"; then
    print_status "OK" "Gaming VM is running"
else
    print_status "ERROR" "Gaming VM is not running"
    echo ""
    print_status "ACTION" "Starting gaming VM now..."
    ./start-gaming-vm.sh &
    sleep 10
    
    if sudo virsh list --state-running | grep -q "win10-gaming"; then
        print_status "OK" "Gaming VM started successfully"
    else
        print_status "ERROR" "Failed to start gaming VM"
        exit 1
    fi
fi

# Check GPU passthrough
print_status "INFO" "Verifying GPU passthrough..."
GPU_STATUS=$(lspci -k -s 02:00.0 | grep "Kernel driver in use" | cut -d':' -f2 | xargs || echo "none")
if [[ "$GPU_STATUS" == "vfio-pci" ]]; then
    print_status "OK" "RTX 4080 properly bound to VFIO for passthrough"
else
    print_status "WARNING" "GPU driver: $GPU_STATUS (should be vfio-pci for passthrough)"
fi

# Create the complete Windows setup script
WINDOWS_SCRIPT="/tmp/complete-diablo4-setup.ps1"

print_status "INFO" "Creating comprehensive Windows VM setup script..."
...
# Check Looking Glass status and start if needed
print_status "INFO" "Setting up Looking Glass connection..."

if ! pgrep -f "looking-glass-client" > /dev/null; then
    print_status "INFO" "Starting Looking Glass client..."
    looking-glass-client -F &
    LG_PID=$!
    sleep 2
    
    if pgrep -f "looking-glass-client" > /dev/null; then
        print_status "OK" "Looking Glass client started successfully"
    else
        print_status "WARNING" "Looking Glass client may not be running properly"
    fi
else
    print_status "OK" "Looking Glass client already running"
fi

# Copy the script to a location accessible from the VM
SHARED_SCRIPT="/tmp/diablo4-complete-setup.ps1"
cp "$WINDOWS_SCRIPT" "$SHARED_SCRIPT"

echo ""
print_status "ACTION" "🚀 EXECUTE THIS NOW!"
echo ""
echo -e "${PURPLE}Windows VM Setup Instructions:${NC}"
echo "1. Connect to your Windows VM (Looking Glass should be open)"
echo "2. Open PowerShell as Administrator in Windows"
echo "3. Copy and run this PowerShell script:"
echo ""
echo -e "${YELLOW}# Copy this entire script and paste in Windows PowerShell:${NC}"
echo ""

# Display the PowerShell script content for easy copying
cat "$WINDOWS_SCRIPT"

echo ""
echo -e "${GREEN}🎯 After running the script in Windows:${NC}"
echo "✅ All applications will be migrated"
echo "✅ GPU detection issues will be fixed"
echo "✅ Gaming optimizations will be applied"
echo "✅ Diablo IV will be ready to play"
echo ""
echo -e "${BLUE}📋 Alternative: Save script and run in Windows${NC}"
echo "The complete script is saved at: $SHARED_SCRIPT"
echo "Copy it to your Windows VM and run as Administrator"

print_status "OK" "Complete Diablo IV setup ready!"
print_status "INFO" "Execute the PowerShell script in your Windows VM to complete setup"
