...
pub async fn get_active_hardware_profile() -> Result<String, String> {
 Ok(match std::fs::read_to_string("/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor") {
 Ok(content) => content.trim().to_string(),
 Err(_) => "balanced".to_string(),
 })
}
...
pub async fn set_hardware_profile(profile_name: String) -> Result<String, String> {
 ...
 "performance" => {
 if let Err(e) = set_cpu_governor("performance").await {
 return Err(format!("Failed to set performance profile: {}", e));
 }
 },
 "power_saver" => {
 if let Err(e) = set_cpu_governor("powersave").await {
 ...
 "balanced" | _ => {
 if let Err(e) = set_cpu_governor("schedutil").await {
...
async fn set_cpu_governor(governor: &str) -> Result<(), String> {
 ...
 if let Err(_) = fs::write(&governor_path, governor).await {
...
pub async fn get_fan_status() -> Result<Vec<FanStatus>, String> {
 ...
 if let Ok(entries) = fs::read_dir(hwmon_path).await {
 for entry in entries.flatten().await {
 if let Ok(name_file) = fs::read_to_string(entry.path()).await {
 let sensor_name = name_file.trim();
 
 // Look for fan input files
 if let Ok(sensor_entries) = fs::read_dir(&entry.path()).await {
 for sensor_entry in sensor_entries.flatten().await {
 let filename = sensor_entry.file_name();
 let filename_str = filename.to_string_lossy();
 
 if filename_str.starts_with("fan") && filename_str.ends_with("_input") {
 if let Ok(rpm_str) = fs::read_to_string(sensor_entry.path()).await {
 if let Ok(rpm) = rpm_str.trim().parse::<u32>().await {
 fans.push(FanStatus {
...
pub async fn get_available_cpu_governors() -> Result<Vec<String>, String> {
 ...
 if let Ok(content) = fs::read_to_string(&governors_path).await {
...
pub async fn get_current_cpu_governor() -> Result<String, String> {
 ...
 if let Ok(content) = fs::read_to_string(&governor_path).await {
