...
pub async fn process_natural_language(query: String) -> Result<String, String> {
 ...
 Ok("For backup optimization, I suggest scheduling automated backups during low-usage periods, using incremental backup modes, and regularly cleaning old backup files. Your system appears to have good memory management, but occasional cleanup can improve responsiveness.".to_string())
 ...
 Ok("Fan management is crucial to system performance. I recommend enabling the 'Variable Fan Speed' option in your BIOS and adjusting the fan speed according to your system's needs. Additionally, you can install an aftermarket cooling solution to further improve airflow.".to_string())
 } else {
